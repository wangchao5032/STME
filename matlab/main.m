...
%% 绘制原始数据
% showlegend=[];
% showlegend = [showlegend, ShowOriginalCluster2Dimension(D)];
% showlegend = [showlegend, ShowOriginalCluster(D)];
% 4、绘制图例
% legend(showlegend,'A-Type','B-Type');
% legend(showlegend,'A-Type','B-Type','C-Type');
% axis([-10,10,-10,10]);
...
distK_sigma_multi = 100; % 点数最多的类型可以放宽k距离的阈值
distK_sigma_times = 3; 
% 注意下文关于簇有效密度的计算会和数据是否是二维或三维数据有关，记得切换。
...
%           [K,V] = convhull(cluster(:,2),cluster(:,3),cluster(:,1));   %当前簇的最小凸包
        ...
        [len,tmp] = size(cluster)
        clusterDensity = len / V ;    %当前簇的密度=当前簇的数量/当前簇的体积
...
% 4、绘制图例
legend(showlegend,'Noise','Cluster1','Cluster2');
