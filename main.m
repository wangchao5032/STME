...
distK_sigma_multi = 10; % 点数最多的类型可以放宽k距离的阈值
distK_sigma_times = 3; 
...
for ii = 1 : length(D(:,1))     %按照第k个邻居距离的升序进行枚举，也就是第i个点一定是目前所有点里k邻域范围最小的。
    ...
    elseif RSTN_free_pts < MinPts
        ...
        type_buf = D(queue, 4); % 簇的有效点：初始队列中所有点+后续加入队列的点中的核心点
        % 计算初始队列中所有点的均值和标准差
...
%             end
            ...
            if length(X2) >= MinPts  % 如果当前点是核心点
                type_buf = [type_buf; D(ptCurrent, 4)]; % 把当前点加入当前队列的有效点列表。这里有bug，有的点会被重复加入！！！！
