...
def GetKN(data, k, t_window):
    ...
    dist_matrix = np.zeros((rows, rows))  # 距离矩阵
    dist_matrix_topK = np.zeros((rows, types+1))  # 所有点的k近邻矩阵：第0列是不考虑类型的k距离，第1列是第1种类型的k距离，以此类推
    ...
    for i in range(rows):
        ...
        dist_matrix[i, invalid_data] = float("inf")
    ...
    for i in range(rows):
        ...
        k_neighbor_idx[i] = dist_i_idx[:k]  # 包含所有类型的第k个最近的距离 对应的 数据编号
        # 查找每个类型的，第k个最近的距离
        for ty in range(types):  # ty类型
            # 计算多类型k距离
            type_ty_data = (data.values[:, 3] == ty)
            type_ty_dist = dist_matrix[i, type_ty_data].tolist()
            dist_matrix_topK[i][ty+1] = sorted(type_ty_dist)[k-1]  # 对所有ty类型的数据和第i个对象的距离排序，查找第k个
...
def STME(data, k, kt, t_window, min_pts, distK_sigma_multi, distK_sigma_times, spatiotemoral=False):
    ...
    for p in priority:  # 按照第k个邻居距离的升序进行枚举，也就是第p个点一定是目前所有点里k邻域范围最小的
        ...
        normed_density = data.iloc[r_neighbor]['cluster'].values.tolist()
        ...
        if len(r_neighbor_free) < min_pts and r_neighbor_most_label != 0:  # 第p个点的refined neighbors中标签为0或-1的点的数目小于MinPts，且出现最多的类型不是0
            data.iloc[p, 4] = r_neighbor_most_label
            continue
        ...
        dist_topK_init = dist_topK[queue]  # 队列中所有点的考虑类型的k距离
        type_init = data.iloc[queue]["type"].values.tolist()  # 簇的有效点1:簇的初始队列中的所有点的类型
        type_following = []  # 簇的有 效点2: 后续加入队列的点中的核心点

        # 计算初始队列中所有点的均值和标准差
        distK_mu = np.average(dist_topK_init[:, 1:], axis=0)  # 计算“队列中所有点的考虑类型的k距离”中，点的 均值
        distK_sigma_init = np.std(dist_topK_init[:, 1:], axis=0) # 计算“队列中所有点的考虑类型的k距离”中，点的 标准差
        # 遍历队列
        while len(queue) > 0:
            ...
            if data.iloc[cur_pt].at['cluster'] > 0:  # 如果当前点被标记过，这个点只可能是作为共享近邻被加入队列了
                continue
            #  (1) 统计当前对列中各类型点的数目
            type_in_cluster = type_init + type_following  # 簇的有效点：初始队列中所有点+后续加入队列的点中的核心点
            type_hist = []  # 统计当前队列中有效点的所有类型
            for ty in range(n_types):
                type_hist.append(type_in_cluster.count(ty))
            # 当前队列的有效点中，如果某种类型的点数目大于MinPts，则认为这些类型是有影响力的
            type_valid = [True if type_hist[ty] >= min_pts else False for ty in range(n_types)]
            # 找出当前队列的有效点中，影响力最大的类型
            dominate_type = type_hist.index(max(type_hist))
            distK_sigma = distK_sigma_init.copy()
            # 影响力最大的类型可以放宽k距离的阈值（为了使模拟数据边界密度较低的点包含进来）
            distK_sigma[dominate_type] = distK_sigma[dominate_type] * distK_sigma_multi
            ...
            if len(cur_RSTN) >= min_pts:  # 如果当前点是核心点
                ...
                for neigh in cur_topK_neighbor:
                    # 计算当前点的邻居的考虑类型的k距离 和 初始簇考虑类型的k距离均值 的差值
                    distK_diff = np.array(abs(dist_topK[:, 1:][neigh] - distK_mu))
    ...
    return data, cluster_normed_density
