function [ ] = ShowCluster( D,real_clusterLabel )
%% 函数描述： 显示聚类效果
%   D：数据集
%% 函数主体
figure();
...
%% 噪声点画小圆点
cluster = D((D(:, 5) == -1), :);
scatter3(cluster(:,2),cluster(:, 3),cluster(:,1),15,'.','k');  
hold on;
...
for i=1:clusterNum
    ...
    scatter3(clusterA(:, 2), clusterA(:, 3), clusterA(:, 1),15,colorTable(real_clusterNum, :),'+');
    scatter3(clusterB(:, 2), clusterB(:, 3), clusterB(:, 1),15,colorTable(real_clusterNum, :),'<');
    scatter3(clusterC(:, 2), clusterC(:, 3), clusterC(:, 1),15,colorTable(real_clusterNum, :),'o');
